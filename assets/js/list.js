$(function() {
    ...
    // 为form表单绑定submit事件, 实现筛选的功能
    $('#form-search').on('submit', function(e) {
        // 先阻止 默认提交行为
        e.preventDefault();
        // 获取表单中选中项的值
        var cate_id = $('[name=cate_id]').val(); // id
    ...
    function renderPage(total) {
        ...
        laypage.render({
            ...
            // 触发jump回调的方式有两种:
            //          1.点击页码的时候
            //          2.只要调用上面的laypage.render方法 [导致死循环]
            jump: function(obj, first) {
                // 可以通过 first 的值，来判断是通过哪种方式，触发的 jump 回调
                // 如果 first 的值为 true，证明是[方式2]触发的, 否则是[方式1]触发的
                console.log(first);
